...
| État de session porté d'un épisode à l'autre (R03 appliqué à l'agent) | **construit, mesuré : 95 % vierge → 87.5 % / 90 % porté sur 40 épisodes avec la recette corrigée (0 % avec l'ancienne) ; épisodes concaténés à l'entraînement : pire (82.5 % / 75 %, décroissant) ; masque d'attention par épisode (`segment_ids`, égal au chemin porté à 1e-4) : 82.5 % / 92.5 % sans décroissance. Ce que l'état ou le registre de session retiennent de ce qu'un épisode a *lu* : la décision de ne pas relire, pas le contenu (2.5–17.5 % sous état porté, six variantes) ([`10_NEXT_ARCHITECTURE.md`](10_NEXT_ARCHITECTURE.md) §2)** |
