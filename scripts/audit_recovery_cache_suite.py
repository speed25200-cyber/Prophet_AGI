...
FP32 on CPU or CUDA. This expands the original single-prefix diagnostic without
relaxing its tolerance or claiming coverage of BF16 or variable depth.
"""
...
def check_prefix(model, ids, loop_k):
    ...
    length = ids.shape[1]
    require(ids.device.type in ('cpu', 'cuda') and ids.shape[0] == 1 and length >= 3, "invalid prefix")
...
def main():
    ...
    if args.reference_scan and args.device != 'cpu':
        parser.error('--reference-scan is a CPU-only diagnostic')
    device = torch.device(args.device)
    require(device.type != 'cuda' or torch.cuda.is_available(), 'CUDA is unavailable; no CPU fallback')
    ...
    if gdn and device.type == 'cuda':
        require(HAS_FLA and all(layer.allow_fused for layer in gdn), 'CUDA GDN suite requires the FLA path')
    ...
    result = {'protocol': 'recovered-development-cache-v1', 'complete': False,
              'scope': 'deterministic development prefixes, FP32, fixed trained depth; device-specific, not deployment certification',
              ...
              'num_threads': torch.get_num_threads(), 'precision': 'float32', 'device': device.type,
              'cuda': torch.version.cuda if device.type == 'cuda' else None,
              'device_name': torch.cuda.get_device_name(device) if device.type == 'cuda' else None,
              'fla': version('fla-core') if gdn and device.type == 'cuda' else None,
              ...
              'gdn_scan': ('fla_chunk32' if device.type == 'cuda' else
                           'sequential_reference' if args.reference_scan else 'chunk64') if gdn else None,
    ...
    for document_sha, tokens in selected:
        for length in args.lengths:
            ...
            paths = check_prefix(model, ids.to(device), result['loop_k'])
